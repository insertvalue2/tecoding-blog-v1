...
public class BoardController {
	...
	// http://localhost:9090/api/board/search/title?searchValue=asdasd
	// db 에서 검색 쿼리 titleContaing
	// /api/board/search/${searchType}?searchValue=${searchValue}
//	@GetMapping("/board/search")
//	public String searchBoard(String searchValue, Model model,
//			@PageableDefault(size = 4, sort = "id", direction = Direction.DESC) Pageable pageable) {
//		System.out.println("searchValue: " + searchValue);
//
//		Page<Board> boards = boardService.searchBoard(searchValue, pageable);
//		// 현재 페이지
//		int nowPage = boards.getPageable().getPageNumber() + 1;
//		// 블럭에 보여줄 시작 페이지
//		int startPage = Math.max(nowPage - 2, 1); // 두 int 값 중 큰 값을 반환합니다. 결과는 정수 값에 가까운 인수입니다.
