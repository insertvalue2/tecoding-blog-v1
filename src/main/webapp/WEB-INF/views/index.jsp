...
<div class="container">
	<div class="m-2 d-flex justify-content-end">
		<form class="form-inline" action="/board/search" method="get">
			<input type="text" class="form-control" placeholder="검색어를 입려하세요"  name="searchValue"  id="search--text" />
				<button type="submit" class="btn btn-warning text-dark"  id="search--title"   >TITLE</button>
		</form>
	</div>
	<br/>
	...
	<c:forEach var="num" items="${pageNumbers}">
		<c:choose>
			<c:when test="${boards.number + 1 eq num}">
				<li class="page-item ${isNowPage}"><a class="page-link" href="/?page=${num - 1}">${num}</a></li>
			</c:when>
			<c:otherwise>
